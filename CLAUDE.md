...
SMS magic-link auth, real NFC bump, live data-pull integrations (currently
simulated in `src/data/datapull.ts`), local/push notifications, unit tests.
Types are shaped to accommodate these. A Supabase-backed data layer exists
(see above) but is single-tenant / keyed to a fixed dev user until real auth
lands — multi-user support needs Supabase Auth + owner-scoped RLS policies.
